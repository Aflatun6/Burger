package collections.stocks.challenge;
...
public class StockList {
    ...
    public int sellStock(String item, int quantity) {
        ...
        if (inStock != null && quantity <= inStock.availableinStock()) {
...
//        return 0;
    ...
    public int reservedStockList(String item, int quantity) {
        StockItem inStock = list.get(item);
        if (inStock != null && quantity >= 0) {
            return inStock.reservStock(quantity);
        }
        return 0;
    }

    public int unreservedStockList(String item, int quantity) {
        StockItem inStock = list.get(item);
        if (inStock != null && quantity >= 0) {
            return inStock.unreservStock(quantity);
        }
        return 0;
    }
